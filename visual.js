  ...
  function redraw() {
    ...
    if (config.use_img) {
      barEnter
        ...
        .attr("width", "40")
        .attr("height", "50")
        .attr("href", d => config.imgs[d.type.replace(/\s+/g,"")]);
    ...
    .attr("id", "img")
      ...
      //   .attr("x", -16)
      //   .attr("cx", d => xScale(xValue(d)) +config.imgs_x)
      //   .attr("cy", 13)
      //   .attr("r", 40 / 2)
      //   .attr("fill-opacity", 1);
    }
    ...
    if (!long) {
      barEnter
        ...
        .style("fill", function(d,i){
          return config.number_color[i%config.number_color.length]
        })
        .transition()
        ...
        .attr("x", d => {
          return xScale(xValue(d)) + config.number_x;
        })
        .attr("y", 22);
    }
    var barUpdate = bar
    ...
    if (!long) {
      barUpdate
        ...
        .duration(2990 * interval_time)
        .attr("x", d => xScale(xValue(d)) + config.number_x);
    }
    avg =
    ...
    if (config.use_img) {
      barExit.select("img").attr("fill-opacity", 0);
    }
  }
